...
def _get_or_create_user(user_id: str, username: str, avatar: str | None) -> dict:
...
def tick_heartbeat_watchdog():
    ...
    for uid, uname in to_drop:
        log_event(f"User '{uname}' dropped after {HEARTBEAT_MISS_LIMIT} missed heartbeats")


def set_quests(user_id: str, username: str, avatar: str | None, quests: list[dict]):
    with _lock:
        u = _get_or_create_user(user_id, username, avatar)
        u["missed_beats"] = 0
        u["last_heartbeat_time"] = datetime.now().strftime("%H:%M:%S")

        incoming_ids = {q.get("id") for q in quests if q.get("id")}

        uncompleted_incoming_ids = {
            q.get("id") for q in quests
            if q.get("id") and not q.get("userStatus", {}).get("completedAt") and not q.get("userStatus", {}).get(
                "claimedAt")
        }
